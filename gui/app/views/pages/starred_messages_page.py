"""
Starred Messages page -- WhatsApp starred/bookmarked messages across all
conversations. Shows message text, sender, conversation, media type,
timestamp, with navigation to source chat.
"""

from __future__ import annotations

from datetime import datetime

from PySide6.QtCore import QModelIndex, Qt, QTimer, Signal
from PySide6.QtGui import QColor, QCursor, QFont
from PySide6.QtWidgets import (
    QAbstractItemView, QApplication, QHBoxLayout, QHeaderView, QLabel,
    QLineEdit, QMenu, QPushButton, QTableView, QVBoxLayout, QWidget,
)

from app.config import format_timestamp
from app.models.base_table_model import BaseLazyTableModel
from app.services.database import Database
from app.services.theme_manager import ThemeManager


_JOINS = """
    FROM message m
    LEFT JOIN media me ON me.message_id = m.id
    LEFT JOIN contact c ON c.id = m.sender_id
    LEFT JOIN conversation conv ON conv.id = m.conversation_id
"""

_WHERE = "m.is_starred = 1"


class StarredMessagesModel(BaseLazyTableModel):
    _columns = [
        ("text_content", "Message"),
        ("sender_name", "Sender"),
        ("phone_number", "Phone / JID"),
        ("conv_name", "Conversation"),
        ("type_label", "Type"),
        ("timestamp", "Starred At"),
    ]

    _base_sql = f"""
        SELECT COALESCE(me.media_caption, m.text_content,
                        '[' || COALESCE(m.type_label, 'media') || ']') AS text_content,
               CASE WHEN m.is_bot_message = 1 AND m.sender_id IS NULL THEN 'Meta AI'
                    WHEN m.from_me = 1 AND m.sender_id IS NULL THEN
                        COALESCE((SELECT value FROM case_metadata WHERE key='device_owner_name'), 'You')
                    ELSE COALESCE(c.resolved_name, c.wa_name,
                         '+' || c.phone_number,
                         REPLACE(c.phone_jid, '@s.whatsapp.net', '+'),
                         conv.display_name, 'Unknown') END AS sender_name,
               CASE WHEN m.from_me = 1 THEN
                    COALESCE((SELECT value FROM case_metadata WHERE key='device_owner_jid'), '')
                    ELSE COALESCE(c.phone_jid,
                         COALESCE(c.phone_number || '@s.whatsapp.net', ''))
                    END AS phone_number,
               COALESCE(conv.display_name, conv.jid_raw_string) AS conv_name,
               COALESCE(m.type_label, '') AS type_label,
               m.timestamp,
               m.id, m.conversation_id,
               COALESCE(conv.display_name, conv.jid_raw_string, '#' || m.conversation_id) AS nav_conv_name,
               m.from_me, m.is_revoked, m.is_edited,
               me.mime_type, me.media_name
    {_JOINS}
    WHERE {_WHERE}"""

    _count_sql = f"SELECT COUNT(*) {_JOINS} WHERE {_WHERE}"
    _default_order = "m.timestamp DESC"

    _TYPE_ICONS = {
        "text": "\U0001F4AC", "image": "\U0001F4F7", "video": "\U0001F3AC",
        "gif": "\U0001F3AC", "audio": "\U0001F3B5", "voice": "\U0001F399",
        "document": "\U0001F4C4", "sticker": "\U0001F36D", "poll": "\U0001F4CA",
        "location": "\U0001F4CD", "live_location": "\U0001F4CD",
        "vcard": "\U0001F464", "view_once_image": "\U0001F441",
        "view_once_video": "\U0001F441", "list_message": "\U0001F4CB",
    }

    def data(self, index: QModelIndex, role: int = Qt.DisplayRole):
        if not index.isValid():
            return None
        row_data = self._data[index.row()]
        col = index.column()

        if role == Qt.DisplayRole:
            raw = row_data[col]
            # Message text — truncate
            if col == 0 and raw:
                s = str(raw).replace("\n", " ")
                return s[:150] + "\u2026" if len(s) > 150 else s
            # Timestamp
            if col == 5 and raw:
                return format_timestamp(raw, "full")
            # Type — icon + label
            if col == 4:
                tl = str(raw) if raw else "text"
                icon = self._TYPE_ICONS.get(tl, "")
                return f"{icon} {tl}" if icon else tl
            # Phone/JID — clean display
            if col == 2 and raw:
                s = str(raw)
                if s.endswith("@s.whatsapp.net"):
                    s = "+" + s.replace("@s.whatsapp.net", "")
                return s
            return str(raw) if raw is not None else ""

        if role == Qt.ForegroundRole:
            tm = ThemeManager.get()
            # Sender name
            if col == 1:
                from_me = row_data[9] if len(row_data) > 9 else False
                if from_me:
                    return QColor("#e65100") if tm.is_light else QColor("#ff9800")
                return QColor("#00695c") if tm.is_light else QColor("#4dd0e1")
            # Conversation
            if col == 3:
                return QColor("#37474f") if tm.is_light else QColor("#90a4ae")
            # Type
            if col == 4:
                return QColor("#7c4dff") if tm.is_light else QColor("#b388ff")
            # Revoked
            if col == 0:
                is_revoked = row_data[10] if len(row_data) > 10 else False
                if is_revoked:
                    return QColor("#ef5350")

        if role == Qt.TextAlignmentRole:
            if col == 5:
                return Qt.AlignRight | Qt.AlignVCenter

        if role == Qt.ToolTipRole:
            if col == 0:
                return str(row_data[col]) if row_data[col] else ""
            if col == 2:
                return str(row_data[col]) if row_data[col] else ""

        return None


class StarredMessagesPage(QWidget):
    conversation_selected = Signal(int, str)
    go_to_message = Signal(int, int)  # conv_id, msg_id — jumps to the specific starred msg

    def __init__(self, parent=None):
        super().__init__(parent)
        self._tm = ThemeManager.get()
        layout = QVBoxLayout(self)
        layout.setContentsMargins(24, 20, 24, 16)
        layout.setSpacing(12)

        # Header
        header = QHBoxLayout()
        title = QLabel("\u2B50  Starred Messages")
        f = QFont(); f.setPointSize(18); f.setBold(True); title.setFont(f)
        header.addWidget(title)
        self._count_label = QLabel("")
        self._count_label.setStyleSheet(self._tm.header_label_style())
        header.addWidget(self._count_label)
        header.addStretch()
        layout.addLayout(header)

        # Info bar
        hint = QLabel(
            "Messages starred by the device owner in WhatsApp  |  "
            "Double-click to go to chat  |  Right-click for options"
        )
        hint.setStyleSheet(self._tm.hint_label_style())
        layout.addWidget(hint)

        # Search box
        search_bar = QHBoxLayout()
        search_bar.setSpacing(8)
        self._search = QLineEdit()
        self._search.setPlaceholderText("\U0001F50D  Search starred messages...")
        self._search.setFixedHeight(36)
        self._search.setClearButtonEnabled(True)
        search_bar.addWidget(self._search, 1)
        layout.addLayout(search_bar)

        # Table
        self._model = StarredMessagesModel()
        self._table = QTableView()
        self._table.setModel(self._model)
        self._table.setSelectionBehavior(QAbstractItemView.SelectRows)
        self._table.setSelectionMode(QAbstractItemView.SingleSelection)
        self._table.setAlternatingRowColors(True)
        self._table.verticalHeader().setVisible(False)
        self._table.setShowGrid(False)
        self._table.setSortingEnabled(True)
        self._table.verticalHeader().setDefaultSectionSize(32)
        self._table.setContextMenuPolicy(Qt.CustomContextMenu)
        self._table.customContextMenuRequested.connect(self._show_context_menu)
        self._table.doubleClicked.connect(self._on_double_click)

        hdr = self._table.horizontalHeader()
        hdr.setSectionResizeMode(0, QHeaderView.Stretch)  # Message
        for col, w in [(1, 150), (2, 180), (3, 180), (4, 90), (5, 170)]:
            hdr.setSectionResizeMode(col, QHeaderView.Fixed)
            hdr.resizeSection(col, w)
        # Hide internal columns
        for hcol in (6, 7, 8, 9, 10, 11, 12, 13):
            self._table.setColumnHidden(hcol, True)
        layout.addWidget(self._table, 1)

        # Search debounce
        self._search_timer = QTimer()
        self._search_timer.setSingleShot(True)
        self._search_timer.setInterval(300)
        self._search_timer.timeout.connect(self._apply)
        self._search.textChanged.connect(lambda: self._search_timer.start())
        self._search.returnPressed.connect(self._apply)
        QTimer.singleShot(50, self._apply)

    def _apply(self):
        parts, params = [], []
        text = self._search.text().strip()
        if text:
            parts.append(
                "(m.text_content LIKE ? OR me.media_caption LIKE ? "
                "OR c.resolved_name LIKE ? OR c.wa_name LIKE ? "
                "OR conv.display_name LIKE ? OR me.media_name LIKE ?)"
            )
            params.extend([f"%{text}%"] * 6)
        self._model.load(where=" AND ".join(parts), params=tuple(params))
        self._count_label.setText(f"{self._model.total_rows:,} starred messages")

    def _get_row_data(self, index: QModelIndex) -> tuple | None:
        if not index.isValid():
            return None
        row = index.row()
        if 0 <= row < len(self._model._data):
            return self._model._data[row]
        return None

    def _on_double_click(self, index: QModelIndex):
        row_data = self._get_row_data(index)
        if row_data:
            # [6]=m.id (msg_id), [7]=conversation_id, [8]=nav_conv_name
            msg_id = row_data[6] if len(row_data) > 6 else 0
            conv_id = row_data[7]
            if conv_id:
                if msg_id:
                    self.go_to_message.emit(int(conv_id), int(msg_id))
                else:
                    conv_name = row_data[8] or f"#{conv_id}"
                    self.conversation_selected.emit(conv_id, conv_name)

    def _show_context_menu(self, pos):
        index = self._table.indexAt(pos)
        row_data = self._get_row_data(index)
        if not row_data:
            return
        menu = QMenu(self)
        menu.setStyleSheet(self._tm.context_menu_style())

        conv_id = row_data[7]
        conv_name = row_data[8] or f"#{conv_id}"
        if conv_id:
            go_chat = menu.addAction("\u2192  Go to Chat")
            go_chat.triggered.connect(
                lambda: self.conversation_selected.emit(conv_id, conv_name)
            )
            menu.addSeparator()

        msg_text = row_data[0] or ""
        if msg_text:
            copy_msg = menu.addAction("Copy Message Text")
            copy_msg.triggered.connect(
                lambda: QApplication.clipboard().setText(str(msg_text))
            )

        phone = row_data[2] or ""
        if phone:
            copy_phone = menu.addAction("Copy Phone / JID")
            copy_phone.triggered.connect(
                lambda: QApplication.clipboard().setText(str(phone))
            )

        sender = row_data[1] or ""
        if sender:
            copy_sender = menu.addAction(f"Copy Sender: {sender}")
            copy_sender.triggered.connect(
                lambda: QApplication.clipboard().setText(str(sender))
            )

        menu.exec(QCursor.pos())

    def refresh_for_timezone_change(self) -> None:
        self._apply()
